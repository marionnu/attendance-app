...
@php
  ...
  $dateObj = !empty($attendance->work_date) ? \Carbon\Carbon::parse($attendance->work_date) : null;
@endphp
  ...
  <form method="POST" action="{{ route('admin.attendance.update', $attendance) }}">
    ...
    <div class="card">
      ...
      <div class="row">
        ...
        <div class="value">
          @if($dateObj)
            <span class="date-year">{{ $dateObj->format('Y年') }}</span>
            <span class="date-md">{{ $dateObj->format('n月j日') }}</span>
          @else
            —
          @endif
        </div>
      ...
      <div class="row">
        ...
        <div class="value">
          <textarea name="note" rows="3" {{ $isPending ? 'disabled' : '' }}>{{ old('note', $noteValue) }}</textarea>
        </div>
      </div>
    </div>
